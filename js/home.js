...
const sounds = {
  ...
  answer: document.getElementById('answerSound')
};
...
// Add answer sound to answer selections
// This will detect clicks on answer options in the question card
document.addEventListener('click', (e) => {
  const answerElement = e.target.closest('[data-answer], .answer-option, .q-card button, .q-card [role="button"]');
  
  // Make sure it's inside the questions section and not a nav button
  const isInQuestions = e.target.closest('#qCard');
  const isNavButton = e.target.closest('#prevBtn, #nextBtn');
  
  if (answerElement && isInQuestions && !isNavButton) {
    // Play answer sound instead of (or in addition to) click sound
    playSound('answer');
  }
});
